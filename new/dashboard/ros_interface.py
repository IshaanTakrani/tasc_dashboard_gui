"""
Handles ROS2 node communication, including publishing inputs and 
receiving video/telemetry data from the Rover.
"""
...
class RosWorker(QThread):
    """
    QThread wrapper that spins the ROS2 node in the background.
    Emits signals to update the GUI safely.
    """
    stats_signal = pyqtSignal(float, float, int, float, bool)
    video_signal = pyqtSignal(QImage)
    telemetry_signal = pyqtSignal(float, float) # Updated to carry two angles
...
class DashboardROSNode(Node):
    """
    ROS2 Node that manages publishers/subscribers for control and video.
    Calculates network statistics (latency, stability).
    """
    ...
    def callback(self, msg):
        try:
            ...
            # Extract both angles
            a1 = data.get("arm_angle_1", 45.0)
            a2 = data.get("arm_angle_2", 0.0)
            self.worker.telemetry_signal.emit(float(a1), float(a2))
